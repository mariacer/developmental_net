...
"""
Run developmental network on CIFAR10
------------------------------------
"""
import numpy as np
import random
...
def load_cifar10(config, path='data'):
    """Load CIFAR10 dataset and return train and valid dataloaders.
    ...
    t_set = datasets.CIFAR10(root=path, train=True, download=True, transform=t)
    t_dl = torch.utils.data.DataLoader(t_set, batch_size=B, shuffle=True)

    v_set = datasets.CIFAR10(root=path, train=False, download=True, transform=t)
...
if __name__=='__main__':
    ...
    config = {'N':[32*32*3, 40, 10],
              ...
              'instances':1,
              'input_reshape':(lambda x:x.view(-1, 32*32*3))} 

    t_dl, v_dl = load_cifar10(config)
